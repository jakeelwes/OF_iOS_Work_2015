...
//  3dFFT_v2
//
//  Library by Blair Neal
//  Edited by Jake Elwes
//
...
void fftShapes::buildMainShape(){
    ...
    if(!pauseMesh){
        if(ofGetFrameNum()%timeStep==0){
            if (fft.getUnScaledLoudestValue()>cutoff){ //dont draw if incoming volume is too low
...
void fftShapes::makeDisc(int xIndex, float yHeight, int zTime){
    ...
    //OUTER RADIUS
    float tempRadius = 0; //spinSpeed*((fft.getNumFFTbins())*meshSpacingWidth/2);//*numRevolutions;//use this if you want it to spiral outwards
    ...
    //float yOffset = th ickness*6*numRevolutions - (sin(ofGetElapsedTimef())-(float)x/(eqCols*meshSpacingWidth)-1/(eqCols*meshSpacingWidth))*(6*thickness); //use this to make it lean left or right as x
    ...
    //    cout<<timePos*spinSpeed*maxMeshLength<<endl;
    
    if(complete && timePos*spinSpeed*maxMeshLength>=543000){
        //        if(complete && timePos*spinSpeed>29.7){ - but meshlength can change it
...
void fftShapes::drawDebug(){
    ...
    ofNoFill();
    ofRect(ofGetWidth()-200, 0, 200, 200);
    ofRect(ofGetWidth()-200, 200, 200, 200);
    ofRect(ofGetWidth()-200, 400, 200, 200);
    ofRect(ofGetWidth()-200, 600, 200, 200);
    
    fft.drawHistoryGraph(ofPoint(ofGetWidth()-200,0), LOW);
    fft.drawHistoryGraph(ofPoint(ofGetWidth()-200,200),MID );
    fft.drawHistoryGraph(ofPoint(ofGetWidth()-200,400),HIGH );
    fft.drawHistoryGraph(ofPoint(ofGetWidth()-200,600),MAXSOUND );
    ofDrawBitmapString("LOW",ofGetWidth()-180,20);
    ofDrawBitmapString("HIGH",ofGetWidth()-180,420);
    ofDrawBitmapString("MID",ofGetWidth()-180,220);
    ofDrawBitmapString("MAX VOLUME",ofGetWidth()-180,620);
...
void fftShapes::setupGUI(){
    ...
    jakeParams.setName("Jake's Main Params");
    jakeParams.add(complete.set("Save on complete", false));
    jakeParams.add(fftNormalize.set("Normalize over history", true));
    jakeParams.add(timeStep.set("Sample frame skip", 2, 1, 30));
    jakeParams.add(exponent.set("Exponent Curve", 3, 1, 3.0 ));
    jakeParams.add(radToSpike.set("Rad To Spike", 0.008, 0, 0.1)); //ratio of the radius of the circle to the size of a spike...needs work
    jakeParams.add(spinSpeed.set("Spin Speed", 0.1, 0, 3)); //1 to revolve normally, higher or lower to love slower or faster around a circle (depends on drawing type)
    jakeParams.add(binAmount.set("Percentage of FFT", 0.066, 0.01, 0.15));
    jakeParams.add(spikeScaleFactor.set("Spike Scale Factor", 1.4,0,4));
    jakeParams.add(cutoff.set("Stop on quiet", 0, 0, 3));
    //this is the longest the mesh can be...too high and the graphics will bog down due to too many points
    ...
    soundParams.add(maxVol.set("Max Volume", 1,1,500));
    soundParams.add(fftBins.set("Num FFT Bins", 64, 8, 96));
    
    soundParams.add(volHistoryLength.set("Norm. Vol. History Length", 600, 5, 800));
    //how often should we sample the sound input? Every frame, or less often? App runs at 60fps, so we just sample every other frame
    ...
    shapeParams.add(meshSpacingDepth.set("Space btwn time pts",46, 1, 50)); //this is how far apart the mesh points are when spaced by time
    ...
    shapeParams.add(maxMeshLength.set("Max Mesh Length", 5000,200,15000));
    ...
    shapeTypes.add(shapeTypeInt.set("Select Type", 0,0,2)); //this really needs to be a dropdown menu...
    shapeTypes.add(shapeName.set("Type", ""));
    
    
    
    camPosParams.setName("Cam Position Controls");
    camPosParams.add(scaleMesh.set("Scale Mesh", 0.26, 0.01, 0.4));
    ...
    colorParams.add(peakColor.set("Peak Color", ofColor(255,255,255,153),ofColor(0,0), ofColor(255)));
    colorParams.add(quietColor.set("Quiet Color", ofColor(42,42,42,188),ofColor(0,0), ofColor(255)));
    colorParams.add(loudColor.set("Loud Color", ofColor(255,255,255,142),ofColor(0,0), ofColor(255)));
    
    colorParams.add(bottomColor.set("Bottom Color", ofColor(244,244,244,61),ofColor(0,0), ofColor(255)));
    
    allParams.add(bShowBars.set("Draw Bars", false));
    allParams.add(bDrawDebug.set("Show Debug", false));
    allParams.add(bShowWireframe.set("Show Wireframe", false));
    allParams.add(pauseMesh.set("Pause Mesh", false));
    
    allParams.add(jakeParams);
